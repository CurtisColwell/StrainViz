...
def map_forces(geometry, force_output):
	...
	vmd_writer(geometry[6:-4] + "/bond_" + os.path.splitext(force_output)[0] + ".tcl", bond_forces_vmd, geometry, bond_min, bond_max, "scripts/vmd_header.tcl")
	vmd_writer(geometry[6:-4] + "/angle_" + os.path.splitext(force_output)[0] + ".tcl", angle_forces_vmd, geometry, angle_min, angle_max, "scripts/vmd_header.tcl")
	vmd_writer(geometry[6:-4] + "/dihedral_" + os.path.splitext(force_output)[0] + ".tcl", dihedral_forces_vmd, geometry, dihedral_min, dihedral_max, "scripts/vmd_header.tcl")
...
def combine_dummies(forces, geometry, force_type):
	...
	vmd_writer(geometry[6:-4] + "/" + force_type + "_total.tcl", new_forces_vmd, geometry, scale_min, scale_max, "scripts/vmd_header.tcl")
...
def combine_force_types(forces, geometry):
	...
	#Write the forces to a .tcl script
	new_forces_vmd, scale_min, scale_max = vmd_norm(new_forces)
	vmd_writer(geometry[6:-4] + "/total_force.tcl", new_forces_vmd, geometry, scale_min, scale_max, "scripts/vmd_header.tcl")